...
if(isset($_CONTEXT['path_parts'][2])) {
    $str = trim($_CONTEXT['path_parts'][2], "\t\n\r\0\x0B\s");
    $str = $str[0];
    $controller_file = "controllers/". $_CONTEXT['path_parts'][1] . "/" . $str . "roduct" . "_controller.php";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['product_id'])) {
            ...
            if (is_array($res)) {
                ...
                if (is_string($res))
                    $_SESSION['error_add_category_to_product'] = $res;
            } else
                $_SESSION['error_add_category_to_product'] = "Empty Product";
            ...
            if (is_array($res)) {
                ...
                $res = SQL_Request($sql, [$_POST['category_name']]);
                if (is_string($res))
                    $_SESSION['category_form_error'] = $res;
            }
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if (isset($_GET['rating']) && isset($_GET['product_id'])) {
            ...
            if (is_array($res)) {
                ...
                $view_data['error']['update_rating'] = $res;
        }
